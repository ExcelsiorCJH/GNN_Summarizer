    ...
    def gat_dataloader(self, features_list, edge_index_list, edge_attr_list, labels_list, batch_size):
        ...
        if self.use_gdc:
            ...
            for data in data_list:
                try:
                    gdc_data_list.append(self.gdc(data))
                except:
                    self.CNT += 1
                    gdc_data_list.append(data)
            
            gat_loader = torch_geometric.data.DataLoader(gdc_data_list, 
                                                         batch_size=batch_size, 
                                                         shuffle=False)
        else:
            gat_loader = torch_geometric.data.DataLoader(data_list, 
                                                         batch_size=batch_size, 
                                                         shuffle=False)
        return gat_loader
